...
def obtain_label_bias(loader, netF, netB, netC, args, label_cnt=0, percen=0.5, last=0, sim_bank=[]):
    ...
    with torch.no_grad():
        ...
        for _ in range(len(loader)):
            ...
            if start_test:
                ...
                all_fea_uni = feas_uniform.float().cpu()
                ...
                all_fea_uni = torch.cat((all_fea_uni, feas_uniform.float().cpu()), 0)
    ...
    z1 = zip(eng_list, idx_list, sim_list)
    sort_list = sorted(z, key=lambda x:(x[0]), reverse=True)
    sort_list_re  = sorted(z1, key=lambda x:(x[0]), reverse=False)
    #print(sort_list)
    sorted_eng_list, sorted_idx_list, sorted_sim_list = zip(*sort_list)
    _, sorted_idx_confi_list, _ = zip(*sort_list_re)
    l = len(sorted_idx_list)

    # first choose top 5% data with largest energy
    sorted_idx_confi_list = sorted_idx_confi_list[:int(0.9*l)]
    ...
    select_confi_loc = []
    select_confi_label = []
    while selected_cnt < 0.4 *l:
        now = sorted_idx_confi_list[selected_cnt]
        if now not in select_confi_loc:
            select_confi_loc.append(now)
            select_confi_label.append(pred_label[now])
            selected_cnt += 1
    ori_acc_cnt = 0
    for i in select_confi_loc:
            if pred_label[i] == all_label[i]:
                ori_acc_cnt += 1
    print("Original clustering acc rate for selected confi samples: {:.2f} % (the higher the better)".\
            format(ori_acc_cnt / len(select_confi_label) * 100))
    ...
    already_labeled_idx_lbl = all_label[already_labeled_idx]
    already_labeled_idx_nei_loc = []
    already_labeled_idx_nei_lbl = []
    all_fea_uni = all_fea_uni.float().cpu().numpy()
    distance = torch.tensor(all_fea_uni) @  torch.tensor(all_fea_uni).t()
    _, idx_near_c = torch.topk(distance, dim=-1, largest=True, k=5)
    idx_near_already =  idx_near_c[already_labeled_idx]
    for index in range(len(already_labeled_idx)):
        already_labeled_idx_nei_loc.append(already_labeled_idx[index])
        already_labeled_idx_nei_lbl.append(already_labeled_idx_lbl[index])
        nei_al = idx_near_already[index]
        near_i = -1
        for idx in range(len(nei_al)):
            if nei_al[idx] != already_labeled_idx[index]:
                near_i = nei_al[idx]
                break
        if near_i != -1:
            if already_labeled_idx[index] in idx_near_c[near_i]:
                already_labeled_idx_nei_loc.append(near_i)
                already_labeled_idx_nei_lbl.append(already_labeled_idx_lbl[index])
    ...
    #print(log_str+'\n')
    return pred_label.astype('int'), torch.tensor(pred_idx), labeled_cnt, thre_w,select_confi_loc,select_confi_label,all_inputs,already_labeled_idx_nei_loc, already_labeled_idx_nei_lbl
...
def train_knn_all(dset_loaders, netF, netC, netB, fea_bank, score_bank, eng_bank, tar_idx, args, optimizer, sim_bank):
    iter_num = 0
    max_iter =  len(dset_loaders)
    while iter_num < max_iter:
        try:
            inputs_test, _, tar_idx = iter_test.next()
        except:
            iter_test = iter(dset_loaders)
            inputs_test, _, tar_idx = iter_test.next()
        inputs_target = inputs_test.cuda()
        features_test = netB(netF(inputs_target))
        output = netC(features_test)
