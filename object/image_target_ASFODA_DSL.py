...
def train_target(args):
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0:
            netF.eval()
            netB.eval()
            mem_label, mem_weight,flag_cls,h_dict,cls_unknown,reli_all_cls,reli_all_cls_common,fea_bank= obtain_label(dset_loaders['tsne'], netF, netB, netC, args,flag,h_dict)
        ...
        lr_scheduler(optimizerU, iter_num=iter_num, max_iter=max_iter)
        ...
        softmax_out_un = nn.Softmax(dim=1)(outputs_un_net)
        entropy_loss_un = torch.mean(loss.Entropy(softmax_out_un))      
        msoftmax_un = softmax_out_un.mean(dim=0)
        gentropy_loss_un = torch.sum(-msoftmax_un * torch.log(msoftmax_un + args.epsilon))
        entropy_loss_un -= gentropy_loss_un
        classifier_loss += entropy_loss_un
        ...
        for index in range(len(tar_idx)):
            flag_ri = False
            can_da_idx = tar_idx[index].tolist()
            if flag_cls_ba[index] != 2:
                mix_pred.append(-1)
                weight_mix[index] = 0.0
                continue
            for i_idx in range(len(reli_all_cls)):
                if  len(reli_all_cls[i_idx]) != 0 and can_da_idx in reli_all_cls[i_idx]:
                    mix_pred.append(i_idx)
                    weight_mix[index] = 1.0
                    lam = np.random.beta(5,5)
                    sel_id = np.random.randint(0,len(reli_all_cls[i_idx]))
                    inputs_mix_hard[index] = lam *inputs_test[index] + (1-lam) * fea_bank[reli_all_cls[i_idx][sel_id]].cuda()
                    flag_ri = True
                    break
            if flag_ri != True:
                mix_pred.append(-1)
        ...
        mix_pred = torch.tensor(mix_pred)
        features_test_mix = netB(netF(inputs_mix_hard.cuda()))
        outputs_test_mix = netCU(features_test_mix)
        outputs_test_mix_un= outputs_test_mix[mix_pred != -1, :]
        pred_mix_un = mix_pred[mix_pred != -1]
        weight_mix_un = weight_mix[mix_pred != -1]
        classifier_loss_mix = CrossEntropyFeatureAugWeight(num_classes=len(cls_unknown))(outputs_test_mix_un, pred_mix_un.cuda(),weight_mix_un.cuda()).float()
        
        classifier_loss += classifier_loss_mix
...
def obtain_label(loader_tsne, netF, netB, netC, args,flag,h_dict):
    ...
    _, idx_near_ana = torch.topk(distance_all, dim=-1, largest=True, k=args.mk)
    ...
    if flag == 0:
        ...
        for index in range(100):
            h_dict[index] = []
        kmeans = KMeans(assign_class, random_state=0).fit(all_fea)
        cluster_label = kmeans.predict(all_fea)

        distance = torch.tensor(kmeans.cluster_centers_).float() @ all_fea.t()
        _, idx_near = torch.topk(distance, dim=-1, largest=True, k=1)

        

        ent_pro = []
        for cls in range(assign_class):
            ent_pro.append(ent[idx_near[cls]].tolist()[0])
            
        sor = np.argsort(-np.array(ent_pro))
        all_sel_loc = []
        index = 0
        
        while index < active_num:
            idx_cls = idx_near[sor[index]].tolist()[0]
            idx_true_lb = all_label[idx_cls].int().tolist()
            h_dict[idx_true_lb].append(idx_cls)
    ...
    for index in range(len(h_dict)):
        if len(h_dict[index]) == 0:
            ...
            for i_idx in range(len(h_dict[index])):
                ...
                nei_can_ids = idx_near_all[re_can_id]
                for n_i_idx in range(len(nei_can_ids)):
                    if nei_can_ids[n_i_idx] not in final_sel_loc:
                        final_sel_loc.append(nei_can_ids[n_i_idx].tolist())
                        final_sel_label.append(index)
    ...
    cls_corr = {}
    for index in range(len(cls_unknown)):
        cls_corr[cls_unknown[index]] = index
    
    disReliUnknownIdx = []
    disReliLocalUnknownIdx = []
    for index in range(len(pred_label_un)):
        if true_unknown_idx[index] in final_sel_loc:
            disReliUnknownIdx.append(true_unknown_idx[index])
            disReliLocalUnknownIdx.append(index)
            pred_label_un[index] = cls_corr.get(final_sel_label[final_sel_loc.index(true_unknown_idx[index])])
    print(np.sum(cls_unknown[pred_label_un[disReliLocalUnknownIdx]] == all_label[disReliUnknownIdx].float().numpy()) / len(all_label[disReliUnknownIdx]))
    ...
    reli_all_cls_common = {}
    for index in range(len(h_dict)):
        reli_all_cls_common[index] = []
    
    
    for index in range(len(h_dict)):
        can_data = h_dict[index]
        cls_reli = []
        if index >= args.class_num and len(h_dict[index]) > 0:
            for i_idx in range(len(can_data)):
                cls_reli.append(can_data[i_idx])
                nei_cls_reli = idx_near_ana[can_data[i_idx]]
                for add_idx in range(len(nei_cls_reli)):
                    if nei_cls_reli[add_idx].tolist() not in cls_reli:
                        cls_reli.append(nei_cls_reli[add_idx].tolist())
            reli_all_cls[cls_corr.get(index)] = cls_reli
        elif index < args.class_num:
            for i_idx in range(len(can_data)):
                cls_reli.append(can_data[i_idx])
                nei_cls_reli = idx_near_ana[can_data[i_idx]]
                for add_idx in range(len(nei_cls_reli)):
                    if nei_cls_reli[add_idx].tolist() not in cls_reli:
                        cls_reli.append(nei_cls_reli[add_idx].tolist())
            reli_all_cls_common[index] = cls_reli
    ...
    return pred_one, weight, flag_cls,h_dict,cls_unknown,reli_all_cls,reli_all_cls_common,all_inputs
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mk', type=int, default=5, help="neighbor numbers")
    ...
    parser.add_argument('--output', type=str, default='./F_ATRE_ALL_New_cls_final/office-home')
    parser.add_argument('--output_src', type=str, default='./SFODA/TSM/office-home')
    ...
    args.savename = 'par_lca_alpha_' + str(args.cls_par)+'_kk_' + str(args.ck)+'_lada_' + str(args.lada)+'_ratio_'+ str(args.ratio)+'_purify_mix_test_all_t_m_k'+ str(args.mk)+'w_all_loss'
