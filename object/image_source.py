...
def cal_acc_oda(loader, netF, netB, netC):
    ...
    return np.mean(acc[:-1]), np.mean(acc), unknown_acc,hos
...
def test_target(args):
    ...
    if args.da == 'oda':
        ...
        log_str = '\nTraining: {}, Task: {}, Accuracy = {:.2f}% / {:.2f}% / {:.2f}% / {:.2f}%'.format(args.trte, args.name, acc_os2, acc_os1, acc_unknown,hos)
    else:
        if args.dset=='VISDA-C':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='./TSM/office')
